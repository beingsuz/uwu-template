...
function generateConstructCode(
  ...
  try {
    switch (construct.type) {
      ...
      default:
        throw new Error(
          `Unknown construct type: ${(construct as TemplateConstruct).type}`,
        );
    }
  } catch (error) {
    if (error instanceof TemplateError) {
...
function generateComponentCode(
  ...
  args.forEach((arg, index) => {
    if (
      ...
      propsEntries.push(
        `${JSON.stringify(index.toString())}: ${generateDataAccessor(arg, dataVar)}`,
      );
    }
  });
  ...
  Object.entries(hash).forEach(([key, value]) => {
    if (
      ...
      propsEntries.push(
        `${JSON.stringify(key)}: ${generateDataAccessor(value, dataVar)}`,
      );
    }
  });
  const propsCode =
...
function generateBlockHelperCode(
  ...
  const hashCode =
    Object.entries(hash).length > 0
      ? `const ${hashName} = {${Object.entries(hash)
          .map(([key, value]) => {
...
function generateConditionCode(condition: string, dataVar: string): string {
  ...
  code = code.replace(
    ...
    (match) => {
      ...
      if (
        ["true", "false", "null", "undefined", "typeof", "instanceof"].includes(
          match,
        )
      ) {
        return match;
...
function parseIfElseStructure(content: string): {
  ...
  if (elseifMatches.length > 0 || elseIndex >= 0) {
    ...
    const firstBoundary =
      elseifMatches.length > 0 ? elseifMatches[0].index : elseIndex;
    if (firstBoundary >= 0) {
...
function generateHelperCall(
  ...
  const helperCode =
    args.length > 0
      ? `const ${varName} = helpers.get(${JSON.stringify(helperName)})?.call(null, ${args
          .map((arg) => {
