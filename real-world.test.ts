...
Deno.test("Real-World Template Performance Verification", async (t) => {
	...
	await t.step("E-commerce Template Performance", async () => {
		...
		const data = {
			...
			products: Array.from({ length: 24 }, (_, i) => ({
				...
				stars: Array.from(
					{ length: 5 },
					(_, j) => j < Math.floor(Math.random() * 5 + 1),
				),
			})),
			pagination: {
				totalPages: 5,
		...
		console.log(
			`   ⚡ E-commerce: ${rendersPerSecond.toLocaleString()} renders/sec`,
		);
		...
		assertGreaterOrEqual(
			rendersPerSecond,
			800,
			"E-commerce template should render at least 800 times per second",
		);
	});
	...
	await t.step("Blog Post Template Performance", async () => {
		...
		console.log(
			`   ⚡ Blog post: ${rendersPerSecond.toLocaleString()} renders/sec`,
		);
		console.log(`   ⏱️  Average: ${avgTime.toFixed(3)}ms per render`);
		...
		assertGreaterOrEqual(
			rendersPerSecond,
			1000,
			"Blog template should render at least 1000 times per second",
		);
	});
	...
	await t.step("Email Template Performance", async () => {
		...
		console.log(
			`   ⚡ Email: ${rendersPerSecond.toLocaleString()} renders/sec`,
		);
		console.log(`   ⏱️  Average: ${avgTime.toFixed(3)}ms per render`);
		...
		assertGreaterOrEqual(
			rendersPerSecond,
			1200,
			"Email template should render at least 1200 times per second",
		);
	});
	...
	await t.step("Performance Summary", () => {
		...
		console.log(
			"✅ E-commerce templates: High-performance product catalogs",
		);
