...
function generateConstructCode(
	...
	try {
		...
		throw new TemplateSyntaxError(
			...
			construct.column,
			templateName,
			construct.line ? lineTracker.getContext(construct.start) : void 0,
		);
	}
}
...
function parseBlockHelperStructure(content) {
	...
	if (elseMatch && elseMatch.index !== void 0) {
		...
		const elseContent = content.slice(elseMatch.index + elseMatch[0].length)
			.trim();
		return {
			mainContent,
			elseContent,
...
function generateExtendsCode(baseTemplateName, templateName) {
	return `
{
